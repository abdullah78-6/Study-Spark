...
const createtoken=(id)=>{
    return jwt.sign({id},process.env.JWT_SECRET2,{expiresIn:"1d"})
}
const Signup=async(req,res)=>{
    ...
    try {
        ...
        if(user){
            return res.json({status:false,message:"Teacher Already Exist"});
        }
        ...
        return res.json({status:true,message:"Teacher Account Created"});
...
const Getprofile=async(req,res)=>{
    try {
        ...
        const user=await Teachermodel.findById(decoded.id).select("email");
        if(!user){
            return res.json({status:false,message:"Teacher Is not There"});
        }
...
const Logout=async(req,res)=>{
    try {
        ...
        return res.json({status:true,message:"Teacher Logout"});
    } catch (error) {
...
const GoogleLogin=async(req,res)=>{
    const {email}=req.body;
      try{  
            const user=await Teachermodel.findOne({email});
            if(!user){
                return res.json({status:false,message:"Teacher DOES NOT EXIST "});
            }
            ...
            return res.json({status:true,email:email,message:" Teacher Google Authentication Complete"});
